...
<head>
	...
	<script type="text/JavaScript" src="admin/pages/life/js/fbEdit.js">
...
<div class="layout">
       ...
       <div class="business1">
        <form id="form" name="form" action="lifeshow_ModifyFb.htm" method="post" enctype="multipart/form-data">
        <input name="lifeinfo.lifeInfoId" type="hidden" value="${lifeinfo.lifeInfoId}"/>
       	<input name="lifeinfo.publishTime" type="hidden" value="${lifeinfo.publishTime}"/>      
        <input name="lifeinfo.modifyTime" type="hidden" value="${lifeinfo.modifyTime}"/>
        <input name="lifeinfo.hot" type="hidden" value="${lifeinfo.hot}"/>
        <input name="lifeinfo.status" type="hidden" value="0"/>
        
        <input id="img1" name="lifeinfo.image" type="hidden" value="${request.tempImage}"/>
        <input id="img2"name="lifeinfo.image" type="hidden" value="${lifeinfo.image}"/>
          ...
          <table width="785" border="0" cellspacing="0" cellpadding="0"  >
            ...
            <tr>
              ...
              <td rowspan="4">
              	<c:if test="${not empty lifeinfo.image}">
              		<c:if test="${empty tempImage}">
              			<img src="UploadFolder/userFiles/${session.loginUserBean.regLogName}/life/${lifeinfo.image}" width="200" height="150"/>
              		</c:if>
              		<c:if test="${not empty tempImage}">
              			<img src="UploadFolder/userFiles/${session.loginUserBean.regLogName}/life/${tempImage}" width="200" height="150"/>
              		</c:if>
             	 </c:if>
            ...
            <tr>
              ...
              <td><input id="file" type="file" name="file" value="${lifeinfo.image}"/>
                <input name="up" type="submit" value="上传图片" onclick="return upform()"/>
              </td>
            </tr>
            <tr>
              <td height="40">产品说明：</td>
              <td>&nbsp;</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <input type="button" name="button2" id="button2" value="取消" onclick="history.go(-2)"/></td>
              <td>&nbsp;</td>
            </tr>
          </table>
        </form> 
